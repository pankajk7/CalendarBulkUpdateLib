 ...
 * of the {@link com.tripadvisor.CalendarView.FluentInitializer} methods
...
public class CalendarView extends FrameLayout {
	...
	private final List<List<WeekCellDescriptor>> mCells = new ArrayList<List<WeekCellDescriptor>>();
	...
	/*** p.k ***/
	private Calendar mMinRangeCal;
	private Calendar mMaxRangeCal;
	...
	public CalendarView(Context context, AttributeSet attrs) {
		...
		View content = layoutInflater.inflate(R.layout.calendar_view, this,
				false);
		addView(content);
		...
		Drawable horizontalDivider = getResources().getDrawable(
				R.drawable.calendar_week_bg_shadow);
		((ImageView) findViewById(R.id.cv_divider))
				.setImageDrawable(horizontalDivider);
		mFullDateFormat = DateFormat
				.getDateInstance(DateFormat.MEDIUM, mLocale);
		if (isInEditMode()) {
	...
	static boolean betweenDates(Date date, Calendar minCal, Calendar maxCal) {
		...
		return (date.equals(min) || date.after(min)) // >= mMinRangeCal
				&& date.before(maxCal.getTime()); // && < mMaxRangeCal
	}

	private static boolean sameMonth(Calendar cal, WeekDescriptor week) {
		return (cal.get(WEEK_OF_YEAR) == week.getWeekInYear() && cal.get(YEAR) == week
				.getYear());
	}
...
//			doSelectDate(selectedDate, cellDescriptor);
	...
	private boolean doSelectDate(Date date, WeekCellDescriptor cell) {
		...
		case SINGLE:
			...
			break;
		default:
			throw new IllegalStateException("Unknown mSelectionMode "
					+ mSelectionMode);
		}
...
//					}
			...
			if (mSelectionMode == RANGE && getSelectedCells().size() > 1) {
				...
				for (List<WeekCellDescriptor> week : mCells) {
					...
					&& getSelectedCells().size() == 1
					&& getSelectedCells().get(0).getRangeState() == FIRST_AND_LAST
					|| mSelectionMode == RANGE
					&& getSelectedCells().size() == 1
	...
	public FluentInitializer init(Date minDate, Date maxDate, Locale locale,
			Date minRangeDate, Date maxRangeDate) {
		...
		/*** p.k ***/
		mMinRangeCal = Calendar.getInstance(locale);
		mMaxRangeCal = Calendar.getInstance(locale);
		...
		mMonthNameFormat = new SimpleDateFormat(getContext().getString(
				R.string.month_name_format), locale);
		mFullMonthNameFormat = new SimpleDateFormat(getContext().getString(
				R.string.header_month_name_format), locale);
		...
		/*** p.k ***/
		mMinRangeCal.setTime(minRangeDate);
		mMaxRangeCal.setTime(maxRangeDate);
		...
		// Now iterate between mMinCal and mMaxCal and build up our list of
		// mWeeks to show.
		mMonthCounter.setTime(mMinCal.getTime());
		final int maxMonth = mMaxCal.get(MONTH);
		final int maxYear = mMaxCal.get(YEAR);
		while ((mMonthCounter.get(MONTH) <= maxMonth // Up to, including the
														// month.
				|| mMonthCounter.get(YEAR) < maxYear) // Up to the year.
				&& mMonthCounter.get(YEAR) < maxYear + 1) { // But not > next
															// yr.
			Date date = mMonthCounter.getTime();
			WeekDescriptor week = new WeekDescriptor(
					mMonthCounter.get(WEEK_OF_YEAR), mMonthCounter.get(YEAR),
					date, mMonthNameFormat.format(date));
			List<WeekCellDescriptor> weekCells = getWeekCells(week,
					mMonthCounter);
	 ...
	 * {@link com.tripadvisor.CalendarView.SelectionMode} to
	 * {@link com.tripadvisor.CalendarView.SelectionMode#SINGLE}. If you want a
	 * different selection mode, use
	 * {@link com.tripadvisor.CalendarView.FluentInitializer#inMode(com.tripadvisor.CalendarView.SelectionMode)}
	 * on the {@link com.tripadvisor.CalendarView.FluentInitializer} this method
	 * returns.
	 * <p/>
	 * The calendar will be constructed using the default mLocale as returned by
	 * {@link java.util.Locale#getDefault()}. If you wish the calendar to be
	 * constructed using a different mLocale, use
	...
	public FluentInitializer init(Date minDate, Date maxDate) {
		return init(minDate, maxDate, Locale.getDefault(),
				mMinRangeCal.getTime(), mMaxRangeCal.getTime());
	}
	...
	private void scrollToSelectedDates() {
		...
		if (selectedIndex != null) {
			...
			// KLT: This was changed since the background drawable was not
			// correct on launch.
	 ...
	 * {@link com.tripadvisor.CalendarView.SelectionMode} this
	 * CalendarPickerView is configured with: if you are in
	 * {@link com.tripadvisor.CalendarView.SelectionMode#SINGLE}, the previously
	 * selected date will be un-selected. In
	 * {@link com.tripadvisor.CalendarView.SelectionMode#MULTIPLE}, the new date
	 * will be added to the list of selected dates.
	 * <p/>
	...
	private void validateDate(Date date) {
		...
		if (date.before(mMinRangeCal.getTime())
				|| date.after(mMaxRangeCal.getTime())) {
			throw new IllegalArgumentException(
	...
	public Date getSelectedDate() {
		return (getSelectedCals().size() > 0 ? getSelectedCals().get(0)
				.getTime() : null);
	}
	...
	public void setLocale(Locale locale) {
		...
		if (mMonthName != null) {
			WeekDescriptor week = getWeeks().get(
	...
	private void setUpHeader() {
		...
		for (int offset = 0; offset < 7; offset++) {
			...
			if (mLocale.getLanguage().equals("en")) {
				dayLabels[offset] = mWeekdayFormatter.format(
						getToday().getTime()).toUpperCase(mLocale);
			} else {
				dayLabels[offset] = mWeekdayFormatter.format(getToday()
						.getTime());
			}
		}
		...
		for (int i = 1, count = mDayNamesHeader.getChildCount(); i < count; i++) {
			...
			if (i < daysPerWeek + 1) {
				int day = Calendar.getInstance(mLocale).get(
	...
	public List<WeekCellDescriptor> getWeekCells(WeekDescriptor week,
			Calendar startCal) {
		...
		for (int c = 0; c < 7; c++) {
			...
			boolean isSelectable = betweenDates(cal, mMinRangeCal, mMaxRangeCal)
			...
			WeekCellDescriptor cellDescriptor = new WeekCellDescriptor(date,
	...
	private boolean isDateSelectable(Date date) {
		return mDateConfiguredListener == null
				|| mDateConfiguredListener.isDateSelectable(date);
	}
	 ...
	 * 
	 * @param listener
	 *            the mListener to set, or null for no reaction
	 */
	 ...
	 * call {@link #selectDate(java.util.Date)} this mListener will not be
	 * notified.
	 * 
	 ...
	 * you call {@link #selectDate(java.util.Date)} this mListener will not be
	 * notified.
	 * 
	...
	private class CellClickedListener implements WeekView.Listener {
		...
		public void handleClick(WeekCellDescriptor cell) {
			...
			if (!betweenDates(clickedDate, mMinRangeCal, mMaxRangeCal)
					|| !isDateSelectable(clickedDate)) {
				if (mInvalidDateSelectedListener != null) {
					mInvalidDateSelectedListener
	...
	public class FluentInitializer {
		/**
		 * Override the {@link com.tripadvisor.CalendarView.SelectionMode} from
		 * the default (
		 * {@link com.tripadvisor.CalendarView.SelectionMode#SINGLE}).
		 */
		public FluentInitializer inMode(SelectionMode mode) {
			mSelectionMode = mode;
			mSelectionModePrevious = mode;
			validateAndUpdate();
			return this;
		}
		 ...
		 * {@link #inMode(com.tripadvisor.CalendarView.SelectionMode)}.
		 */
	...
	/**
	 * <p>
	 * This is a specialized adapter for creating a list of mWeeks with
	 ...
	 * arbitrary number of mWeeks at a time.
	 * </p>
	 */
	private class WeeksAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			weekView.init(getWeeks().get(position), mCells.get(position),
					mDisplayOnly, mFocusedMonth);
			return weekView;
		}
	...
	private class DefaultOnInvalidDateSelectedListener implements
		...
		public void onInvalidDateSelected(Date date) {
			String errMessage = getResources().getString(R.string.invalid_date,
					mFullDateFormat.format(mMinRangeCal.getTime()),
					mFullDateFormat.format(mMaxRangeCal.getTime()));
